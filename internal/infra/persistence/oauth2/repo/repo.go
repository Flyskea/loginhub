...
func (o *OAuth2ProviderRepo) Update(ctx context.Context, provider *entity.ProviderInfo) error {
	sb := sqlbuilder.NewUpdateBuilder()
	assignment := make([]string, 0, 3)
	if provider.ClientID != "" {
		assignment = append(assignment, sb.Assign("client_id", provider.ClientID))
	}
	if provider.ClientSecret != "" {
		assignment = append(assignment, sb.Assign("client_secret", provider.ClientSecret))
	}
	if provider.RedirectURL != "" {
		assignment = append(assignment, sb.Assign("redirect_url", provider.RedirectURL))
	}
	sql, args := sb.Update(po.OAuth2ProviderTableName).
		Set(assignment...).
		Where(sb.Equal("id", provider.ID)).Build()
	return data.GromToError(o.txm.DB(ctx).Debug().Exec(sql, args...).Error)
}
